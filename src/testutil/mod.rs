...
// NOT consensus: purely construction utilities.
...
/// Turn on test bypass knobs used by tests/drivers.
///
/// IMPORTANT: do NOT gate this behind cfg(test), because a binary (reorg_crash_driver)
/// imports it in normal builds.
pub fn set_test_env() {
    // If your pow/index bypass uses different names, change them here only.
...
/// Coinbase used by tests: must match your consensus coinbase format (height-unique).
pub fn make_coinbase(height: u64) -> Transaction {
    coinbase(test_miner(), INITIAL_REWARD, height)
}

/// Minimal merkle suitable for tests (pair-hash, duplicate last).
...
/// Mine a header by scanning nonce until pow_ok(...).
/// If bypass env is set, skip mining and keep nonce=0.
