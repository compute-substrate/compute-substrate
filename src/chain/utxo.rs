...
pub fn validate_and_apply_block(
    ...
    let blk_bytes = crate::codec::consensus_bincode().serialize(block)?.len();
    ...
    db.undo.insert(k_undo(&bh), crate::codec::consensus_bincode().serialize(&undo)?)?;
    Ok(undo)
}
...
pub fn undo_block(db: &Stores, block_hash: &[u8; 32]) -> Result<()> {
    ...
    let undo: UndoLog = crate::codec::consensus_bincode().deserialize::<UndoLog>(&v)?;
...
fn validate_tx_structure_noncoinbase(tx: &Transaction) -> Result<()> {
    ...
    // serialized size cap
    let tx_bytes = crate::codec::consensus_bincode().serialize(tx)?.len();
