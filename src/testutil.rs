// src/testutil.rs
//
// Test + fuzz helpers used by integration tests and the reorg crash driver.
// These helpers are NOT consensus; they exist only to construct deterministic chains.
...
/// Set environment flags used by tests/drivers.
///
/// Keep this function cheap and deterministic.
/// If you’re using a PoW bypass path gated by env vars, turn it on here.
pub fn set_test_env() {
    // used by your pow/index bypass logic (whatever you named it)
    std::env::set_var("CSD_TEST_BYPASS_POW", "1");
    // allow tests to create their own “genesis” without matching params::GENESIS_HASH
...
/// Coinbase used by tests: must match your consensus coinbase signature format
/// (you already fixed uniqueness via height).
pub fn make_coinbase(height: u64) -> Transaction {
    coinbase(test_miner(), INITIAL_REWARD, height)
}

/// Minimal merkle suitable for tests (Bitcoin-ish pair hash, duplicate last).
...
/// Mine a header by scanning nonce until pow_ok(...) is true.
/// If test PoW bypass env is set, we skip mining and keep nonce=0.
fn mine_header(mut hdr: BlockHeader) -> Result<BlockHeader> {
    // If your codebase bypasses PoW in tests, don’t waste time mining here.
    if std::env::var("CSD_TEST_BYPASS_POW").is_ok() {
        return Ok(hdr);
    }

    // Otherwise do a bounded nonce scan.
...
/// Returns Vec of block hashes by height.
pub fn build_chain(db: &Stores, n: u64, start_time: u64) -> Result<Vec<Hash32>> {
    let mut out = Vec::with_capacity(n as usize);
    let mut prev = [0u8; 32];

    for h in 0..n {
        let t = start_time + (h * 60); // deterministic spacing
        let bh = apply_mined_block(db, prev, h, t).with_context(|| format!("apply h={h}"))?;
        out.push(bh);
        prev = bh;
    }
...
/// Returns hashes for fork blocks (heights fork_height..fork_height+fork_len-1).
pub fn build_fork(
    db: &Stores,
    base_hashes: &[Hash32],
    fork_height: u64,
    ...
    for i in 0..fork_len {
        let h = fork_height + i;
        let t = start_time + (h * 60) + 17; // distinct from base but monotonic
        let bh = apply_mined_block(db, prev, h, t).with_context(|| format!("apply fork h={h}"))?;
        out.push(bh);
        prev = bh;
    }
