...
async fn live_timeout_stall_peer_falls_back_to_good_peer() -> Result<()> {
    ...
    // connect -> GetTip -> GetHeadersByLocator -> GetBlock
    // -> stall timeout (~10s) -> requeue -> fallback -> fetch from good peer -> reorg
    ...
    assert_eq!(
        tip_sync, tip_good,
        "sync node should converge to the good peer's heavy tip after actual block-request timeout fallback"
    );
