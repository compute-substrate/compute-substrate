...
#[test]
fn rejects_tx_with_duplicate_input_outpoint_inside_same_tx() -> Result<()> {
    ...
    let start_time = 1_701_600_000u64;
    ...
    let prevout = OutPoint {
        txid: csd::crypto::txid(&parent_block.txs[0]),
        vout: 0,
    };
    ...
    let bad_tx = Transaction {
        ...
        inputs: vec![
            ...
            TxIn {
                prevout, // intentional duplicate
                script_sig: vec![0u8; 99],
            },
        ],
        outputs: vec![TxOut {
            value: input_value - 10_000,
    ...
    let err = validate_and_apply_block(&db, &blk, epoch_of(height), height)
        .expect_err("block with duplicate input outpoint inside one tx must be rejected");

    let msg = format!("{err:#}");
    assert!(
        msg.contains("duplicate input outpoint"),
