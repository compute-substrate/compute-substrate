...
use csd::types::{Block, Transaction, TxOut};
...
#[test]
fn rejects_block_with_coinbase_multiple_outputs() -> Result<()> {
    ...
    let miner = h20(0x55);
    let shared_len = 7u64;
    let start_time = 1_702_500_000u64;
    ...
    let original_value = cb.outputs[0].value;
    cb.outputs = vec![
        TxOut {
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("coinbase with multiple outputs must be rejected");

    let msg = format!("{err:#}");
    assert!(
        msg.contains("coinbase must have 1 output"),
