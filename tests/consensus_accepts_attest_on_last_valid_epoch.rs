...
#[test]
fn accepts_attest_on_last_valid_epoch() -> Result<()> {
    ...
    let start_time = 1_701_900_000u64;
    ...
    // Boundary case: proposal expires exactly this epoch.
    let expires_epoch = current_epoch;
    ...
    let propose_tx = Transaction {
        ...
        app: AppPayload::Propose {
            ...
            uri: "https://example.com/boundary".to_string(),
            expires_epoch,
        },
    };
    ...
    // Attest tx in same block, at epoch == expires_epoch, which must be accepted.
    ...
    let attest_txid = txid(&attest_tx);
    ...
    let txs = vec![cb, propose_tx.clone(), attest_tx.clone()];
