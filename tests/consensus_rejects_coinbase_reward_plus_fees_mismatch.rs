...
#[test]
fn rejects_block_with_coinbase_reward_plus_fees_mismatch() -> Result<()> {
    let tmp = TempDir::new()?;
    let db = open_db(&tmp)?;
    ...
    let fee = 5_000u64;
    let spend_tx = make_signed_tx(spend_prevout, input_value, fee, h20(0x44));

    let height = shared_len;

    // Intentionally wrong: reward + fee + 1
    let wrong_coinbase_value = csd::params::block_reward(height) + fee + 1;

    let cb = csd::chain::mine::coinbase(miner_block, wrong_coinbase_value, height, None);
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("block with wrong coinbase reward+fees must be rejected");
