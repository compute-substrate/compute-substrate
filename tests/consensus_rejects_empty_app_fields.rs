...
#[test]
fn rejects_propose_with_empty_domain() -> Result<()> {
    let tmp = TempDir::new()?;
    let db = open_db(&tmp)?;
    ...
    let cb = csd::chain::mine::coinbase(
        miner_block,
        csd::params::block_reward(height) + csd::params::MIN_FEE_PROPOSE,
        height,
        None,
    );
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("empty propose domain must be rejected");

    let msg = format!("{err:#}");
    assert!(msg.contains("propose domain empty"), "unexpected error: {msg}");

    Ok(())
}

#[test]
fn rejects_propose_with_empty_uri() -> Result<()> {
    let tmp = TempDir::new()?;
    let db = open_db(&tmp)?;

    let miner_shared = signer_addr(SK);
    let miner_block = h20(0xAB);

    let shared_len = 7u64;
    let start_time = 1_701_500_100u64;
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("empty propose uri must be rejected");

    let msg = format!("{err:#}");
    assert!(msg.contains("propose uri empty"), "unexpected error: {msg}");

    Ok(())
}

#[test]
fn rejects_propose_with_zero_payload_hash() -> Result<()> {
    let tmp = TempDir::new()?;
    let db = open_db(&tmp)?;

    let miner_shared = signer_addr(SK);
    let miner_block = h20(0xAC);

    let shared_len = 7u64;
    let start_time = 1_701_500_200u64;
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("zero payload_hash must be rejected");

    let msg = format!("{err:#}");
    assert!(
        msg.contains("propose payload_hash is all-zero"),
...
#[test]
fn rejects_attest_with_zero_proposal_id() -> Result<()> {
    let tmp = TempDir::new()?;
    let db = open_db(&tmp)?;

    let miner_shared = signer_addr(SK);
    let miner_block = h20(0xAD);

    let shared_len = 7u64;
    let start_time = 1_701_500_300u64;
    ...
    let cb = csd::chain::mine::coinbase(
        miner_block,
        csd::params::block_reward(height) + csd::params::MIN_FEE_ATTEST,
        height,
        None,
    );
    ...
    let err = persist_index_apply_block(&db, &blk, height)
        .expect_err("zero proposal_id must be rejected");

    let msg = format!("{err:#}");
    assert!(
        msg.contains("attest proposal_id is zero"),
        "unexpected error: {msg}"
    );
